...
public class View extends JPanel{
	...
	public View(Controller controller) {
		...
		this.textBoard = new JTextField[9][9];
	...
	void setActionListener(Controller controller) {
		...
		case INPUTSCREEN: 
            ...
            for(int row = 0; row<9 ;row++) {
		        for(int col = 0; col<9 ;col++) {
		        		textBoard[row][col].addActionListener(listener);
		        }
			}
            reset.addActionListener(listener);
			checkSolution.addActionListener(listener);
    ...
    protected void getTextInput(Controller controller) {
		gamePanel.removeAll();
		gamePanel.validate();
		// The game itself, a grid of n x n tiles.
		JPanel game = new JPanel(new GridLayout(9,9));
		for(int row = 0; row<9 ;row++) {
	        for(int col = 0; col<9 ;col++) {
	            textBoard[row][col] = new JTextField();
	            textBoard[row][col].setPreferredSize(new Dimension(resolution*2, resolution*2));
	            textBoard[row][col].setText("");
	            game.add(textBoard[row][col]);
	...
	void resetButton(Controller controller) {
        System.out.println("controller.gameState: " + controller.gameState);
		switch(controller.gameState) {
		case TITLESCREEN: 
			setup(controller);
			setActionListener(controller);
	...
	public void update(ArrayList<Integer> playerMove, Integer integer) {
        // System.out.println("playerMove: " + String.valueOf(playerMove.get(0)) + ", " + String.valueOf(playerMove.get(1)));
        // System.out.println("integer: " + String.valueOf(integer));
		board[playerMove.get(0)][playerMove.get(1)].setFont(new Font("Arial", Font.BOLD, 30));
		if(integer == null) {
